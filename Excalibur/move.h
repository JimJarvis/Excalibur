...
public:
	...
	void setPiece(uint piece) { mov &= 0xffff0fff; mov |= (piece & 0x0000000f) << 12; }
	uint getPiece() { (mov >> 12) & 0x0000000f; }
	// bits 16 to 19, 4 bits for captured piece, if any
	void setCapt(uint capture) { mov &= 0xfff0ffff; mov |= (capture & 0x0000000f) << 16; }
	uint getCapt() { return (mov >> 16) & 0x0000000f; }
	// bits 20 to 23, promotion type
	void setPromo(uint promotion) { mov &= 0xff0fffff; mov |= (promotion & 0x0000000f) << 20; }
	uint getPromo() { return (mov >> 20) & 0x0000000f; }
	// bits 24 to 25, special flags: 01 for O-O, 10 for O-O-O, 11 for en-passant
	void setKCastle() { mov &= 0x0ffffff; mov |= 0x1000000; }
	void setQCastle() { mov &= 0x0ffffff; mov |= 0x2000000; }
	void setEP() { mov &= 0x0ffffff; mov |= 0x3000000; }
	...
	// special queries
	bool isKCastle() { return (mov & 0x1000000) == 0x1000000; }
