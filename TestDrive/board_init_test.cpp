...
Position pos;
#define r(str) pos.rook_attack(str2sq(str))
...
TEST(Board, FEN)
{
	...
	ASSERT_EQ(pos.pieceCount[W][KNIGHT], 2);
	ASSERT_EQ(pos.pieceCount[W][BISHOP], 1);
	ASSERT_EQ(pos.pieceCount[W][ROOK], 0);
	ASSERT_EQ(pos.pieceCount[W][QUEEN], 0);
	ASSERT_EQ(pos.pieceCount[B][PAWN], 5);
	ASSERT_EQ(pos.pieceCount[B][KING], 1);
	ASSERT_EQ(pos.pieceCount[B][KNIGHT], 2);
	ASSERT_EQ(pos.pieceCount[B][BISHOP], 2);
	ASSERT_EQ(pos.pieceCount[B][ROOK], 2);
	ASSERT_EQ(pos.pieceCount[B][QUEEN], 1);
}
...
TEST(Board, Sliders1)
{
	...
	for (int i = 0; i < N; i++)
	{
		fout << "0x" << hex << cb.rook_attack(i) << "ull" << (i==63 ? "};\n" : ", ") ;
	}
	*/
	pos.parseFEN("r1bqk3/pp1p1p1p/8/2N3p1/1q1bP2b/8/P1P1PPPP/2BQKB1R w Kq - 30 17");
	...
	for (int i = 0; i < SQ_N; i++)
		ASSERT_EQ(rook_assertion[i], pos.rook_attack(i));
	...
	for (int i = 0; i < SQ_N; i++)
		ASSERT_EQ(bishop_assertion[i], pos.bishop_attack(i));
}
TEST(Board, Sliders2)
{
	pos.parseFEN("2r3k1/3q2p1/3p4/1p1PpPB1/1b2P3/1n3B2/R6K/5Q2 b - - 10 48");
